...
class StrucDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return tuple(torch.tensor(item, dtype=torch.long) for item in output)
...
def train(args, train_dataset, model, tokenizer):
    ...
    scheduler = get_cosine_with_hard_restarts_schedule_with_warmup(optimizer=optimizer, num_warmup_steps=int(args.warmup_ratio * t_total), num_training_steps=t_total, num_cycles=args.num_cycles)
    if args.fp16:
        try:
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if args.local_rank == -1 and args.evaluate_during_training:
                        results = evaluate(args, model, tokenizer, max_depth=50, prefix=str(global_step))
...
def load_and_cache_examples(args, tokenizer, max_depth=50, evaluate=False, output_examples=False):
    ...
    all_xpath_tags_seq = [f.xpath_tags_seq for f in features]
    all_xpath_subs_seq = [f.xpath_subs_seq for f in features]
    if evaluate:
        all_feature_index = np.arange(len(all_input_ids))
...
def main():
    ...
    parser.add_argument("--evaluate_on_saving", action='store_true',
                        help="Run evaluation during training at each logging step.")
    ...
    if args.local_rank == -1 or args.no_cuda:
        device = torch.device("cuda" if torch.cuda.is_available() and not args.no_cuda else "cuda")
